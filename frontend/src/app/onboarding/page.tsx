            ...
            <div
              className={cn(
                ...
                i < current
                  ...
                  : i === current
                  ? "bg-indigo-500/20 border-indigo-500/50 text-indigo-700 dark:text-indigo-300 ring-2 ring-indigo-500/20"
            ...
            <div className="hidden sm:block">
              <span
                className={cn(
                  ...
                  i === current ? "text-slate-900 dark:text-white" : i < current ? "text-emerald-600 dark:text-emerald-400" : "text-slate-400 dark:text-slate-500"
                )}
              >
                {step.title}
              </span>
              <span className="text-[10px] text-slate-400 dark:text-slate-500 font-mono">{step.subtitle}</span>
            </div>
          </div>
          {i < STEPS.length - 1 && (
            <div
              className={cn(
                ...
                i < current ? "bg-emerald-500/40" : "bg-slate-200 dark:bg-slate-800"
              )}
            />
          )}
        </div>
              ...
              <CardContent className="p-6 sm:p-7 space-y-5">
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-2.5 text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-600 focus:outline-none focus:border-indigo-500/50 transition-all"
                  />
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="w-full border-2 border-dashed border-slate-300 dark:border-slate-800 rounded-2xl p-8 flex flex-col items-center justify-center hover:border-indigo-500/40 hover:bg-slate-100 dark:hover:bg-slate-900/40 transition-all relative cursor-pointer bg-slate-50 dark:bg-slate-950/60">
                    ...
                    <span className="text-[10px] text-slate-500 dark:text-slate-400 mt-1">Accepts standard PDF resume</span>
                    {file && (
              ...
              <CardContent className="p-6 sm:p-7 space-y-5">
                ...
                <div>
                  <label className="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2 flex items-center gap-1.5">
                    <Sparkles className="w-3.5 h-3.5 text-amber-500 dark:text-amber-400" /> Parsed Candidate Skills
                  </label>
                  <div className="flex flex-wrap gap-2 mb-3">
                    {editSkills.map((skill) => (
                      <span
                        key={skill}
                        className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-indigo-500/10 border border-indigo-500/25 text-indigo-700 dark:text-indigo-300 text-xs font-semibold"
                      >
                        {skill}
                        <button
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-2 text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-600 focus:outline-none focus:border-indigo-500/50"
                    />
                    <button
                      onClick={addSkill}
                ...
                <div>
                  <label className="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2 flex items-center gap-1.5">
                    <Briefcase className="w-3.5 h-3.5 text-cyan-600 dark:text-cyan-400" /> Career History
                  </label>
                  {profile.experience.map((exp, idx) => (
                    <div key={idx} className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 mb-2">
                      <div className="flex justify-between items-center">
                        <span className="text-xs font-bold text-slate-900 dark:text-white">{exp.role}</span>
                        <span className="text-[10px] text-slate-500 dark:text-slate-400 font-mono">
                          {exp.start_date} — {exp.end_date}
                        </span>
                      </div>
                      <span className="text-xs text-indigo-600 dark:text-indigo-400 font-medium">{exp.company}</span>
                    </div>
                  ))}
                </div>
                ...
                <div className="flex gap-3 pt-3">
                  <button
                    onClick={() => setStep(0)}
                    className="flex-1 py-3 rounded-full border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white font-bold text-xs transition-colors cursor-pointer"
                  >
                    Back
                  </button>
                  <button
          ...
          >
            <Card className="text-center py-16">
              <CardContent className="space-y-4">
                <div className="w-16 h-16 rounded-full bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-center mx-auto text-emerald-600 dark:text-emerald-400 shadow-xl shadow-emerald-500/10">
                  <Check className="w-8 h-8" />
                </div>
                <h2 className="text-2xl font-extrabold text-slate-900 dark:text-white">Autonomous Agents Online!</h2>
                ...
                <p className="text-[11px] text-slate-400 dark:text-slate-500 font-mono animate-pulse">
