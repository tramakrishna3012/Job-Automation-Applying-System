...
async def generate_personalized_cold_email(contact_name: str, company: str, state: ApplicationState) -> str:
    """Generates personalized cold email via Modal Qwen AI Gateway."""
    profile = state.get("user_profile")
    target_role = state.get("target_role", "Software Engineer")
    skills = ", ".join(profile.skills) if profile and hasattr(profile, "skills") else "Software Development, AI, Cloud Architecture"
    
    prompt = f"Candidate Name: {profile.name if profile and hasattr(profile, 'name') else 'Candidate'}\nTarget Role: {target_role}\nTarget Company: {company}\nRecipient: {contact_name}\nKey Skills: {skills}"
...
def send_cold_email(contact_name: str, email: str, company: str, state: ApplicationState):
    ...
    try:
        ...
        log_telemetry("Communicator", f"Cold email {status} for {contact_name} at {company}", user_id=user_id)
    except Exception as e:
...
def run_communicator(state: ApplicationState) -> ApplicationState:
    console.print("\n[bold blue]--- Phase 5: Timed Cold Emails via Modal Qwen AI Gateway ---[/bold blue]")
    user_id = state.get("user_id")
    ...
    if not df.empty and 'Email' in df.columns and 'Contact Name' in df.columns and 'Company' in df.columns:
        ...
        log_telemetry("Communicator", f"Scheduling cold email outreach campaign for {len(df)} contacts", user_id=user_id)
        ...
        console.print("[green]Cold email outreach campaign scheduled successfully with anti-spam pacing![/green]")
    else:
